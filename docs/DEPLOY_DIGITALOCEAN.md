...
| `Unexpected token '<', "<!DOCTYPE"...` or “HTML instead of JSON” | Often an **HTTP 504 gateway timeout** (body is HTML): Earth Engine took too long. **Production builds use a faster EE profile** automatically (`NODE_ENV=production`). Redeploy the latest app. If you still see 504, use a larger instance or avoid **`EE_FULL_COMPUTE=1`**. Also check wrong **`GOOGLE_APPLICATION_CREDENTIALS_JSON`** (must be raw JSON starting with `{`, one line). |
